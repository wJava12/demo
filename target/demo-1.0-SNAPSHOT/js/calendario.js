...
function muestravalor(x,hora2){
    let t=hora2.split(":");

    fechaHoy=new Date(2022,6,29,t[0],t[1],t[2]);
    ...
    const diasTotales=calendar.map(({daysOfMonth,monthName})=>{
        ...
        if (mes===monthName.valueOf()){
            ...
            for (let i = 0; i <11; i++) {
                ...
                for (let i = 0; i < total; i++) {
                    fila+="<td>"+
                        "<input type='submit'  class='btn btn-success' style='font-size: 10px' id='horita' onclick='(Hora("+i+","+hora+","+fechaHoy.getMinutes()+"))' value='"+hora+':'+fechaHoy.getMinutes()+"'>"+
                        "</td>";
                    // console.log(td);
                }
                fila+="</tr>";
